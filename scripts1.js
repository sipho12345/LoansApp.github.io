$(function() {
	...
	$.extend(Calc.prototype, {
		...
		calculate: function(event){
			...
			if(this.$amount.val()<1001){
			var payment = (+this.$amount.val() * ((this.$percent.val()*this.$slider.val())) / 100)+ +this.$amount.val()+(this.$amount.val())*0.165+50;
			console.log(payment);


			this.$results.fadeIn();
			this.$initamount.text(this.$amount.val());
			this.$pay.text(payment.toFixed(2) + this.currency);
			this.$monthPay.text( (payment / this.$slider.val()).toFixed(2) + this.currency);
			this.$fees.text((this.$amount.val())*0.165);
			this.$interest.text((this.$amount.val()*0.0018*this.$slider.val()).toFixed(2));
			...
			{
				var payment = (+this.$amount.val() * ((this.$percent.val()*this.$slider.val())) / 100)+ +this.$amount.val()+(this.$amount.val())*0.10+50;
			console.log(payment);
			...
			this.$pay.text(payment.toFixed(2) + this.currency);
			this.$monthPay.text( (payment / this.$slider.val()).toFixed(2) + this.currency);
			this.$fees.text((this.$amount.val())*0.10);
			this.$interest.text((this.$amount.val()*0.0018*this.$slider.val()).toFixed(2));
...
var yValues = [50, this.$amount.val()*0.165, (this.$amount.val()*0.0018*this.$slider.val()).toFixed(2),this.$amount.val()];
var barColors = [
  "#b91d47",
  "#00aba9",
  "#2b5797",
